...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(docketDetail: DocketDetail) = apply {
            id = docketDetail.id
            assignedTo = docketDetail.assignedTo
            caseName = docketDetail.caseName
            cause = docketDetail.cause
            court = docketDetail.court
            courtId = docketDetail.courtId
            dateFiled = docketDetail.dateFiled
            dateTerminated = docketDetail.dateTerminated
            docketNumber = docketDetail.docketNumber
            natureOfSuit = docketDetail.natureOfSuit
            pacerCaseId = docketDetail.pacerCaseId
            parties = docketDetail.parties.map { it.toMutableList() }
            url = docketDetail.url
            additionalProperties = docketDetail.additionalProperties.toMutableMap()
        }
     ...
     *   expected type.
     */
    fun validate(): DocketDetail = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun toString() =
        "DocketDetail{id=$id, assignedTo=$assignedTo, caseName=$caseName, cause=$cause, court=$court, courtId=$courtId, dateFiled=$dateFiled, dateTerminated=$dateTerminated, docketNumber=$docketNumber, natureOfSuit=$natureOfSuit, pacerCaseId=$pacerCaseId, parties=$parties, url=$url, additionalProperties=$additionalProperties}"
}
