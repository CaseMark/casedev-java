...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { CasedevInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw CasedevInvalidDataException("`$name` is not set")
            is JsonNull -> throw CasedevInvalidDataException("`$name` is null")
            else -> throw CasedevInvalidDataException("`$name` is invalid, received $this")
        }
